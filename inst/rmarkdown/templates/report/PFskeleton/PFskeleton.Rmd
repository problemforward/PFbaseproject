...
## Session Info
The goal is to have this printed to a separate document and saved in same folder as the report with a timestamp.

This section can be safely ignored. It is for Problem Forward internal use to be able to debug and undertand what versions of all the software we're using, so if there is an error later we can track down what caused it. 

This report was compiled at `r Sys.time()`. The session information is:
